...
实现基于SVD的图像压缩与协同过滤推荐，包括相似度计算（欧氏距离、
皮尔逊相关系数、余弦相似度）、基于SVD的评分估计与图像重构。
"""
import os
# -*- coding: utf-8 -*-
"""
...
def printMat(inMat, thresh=0.8):
    myl = [[] for _ in range(32)]  # 初始化32个空列表
    ...
    for i in range(32):
        ...
        # print('')    
    # 转置矩阵把行变成列
    transposed_myl = np.transpose(myl)
    for k in range(32):
        print(transposed_myl[k])
...
def imgCompress(numSV=3, thresh=0.8):
    myMat = imgLoadData(os.path.join(os.path.dirname(os.path.abspath(__file__)), '0_5.txt'))
    # print(myMat)
