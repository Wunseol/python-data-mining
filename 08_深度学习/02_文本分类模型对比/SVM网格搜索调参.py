...
使用GridSearchCV对SVM超参数（C、kernel、gamma）进行网格搜索，
在训练集子集上调参后用全量数据训练，评估分类性能并绘制ROC曲线。
"""
import os
import h5py
...
if __name__ == '__main__':
    ...
    param_grid = {'C': [0.1, 1, 10], 'kernel': ['linear'], 'gamma': ['scale']}
    grid_search = GridSearchCV(svm_classifier, param_grid, cv=5, scoring='accuracy', verbose=2, n_jobs=-1)
